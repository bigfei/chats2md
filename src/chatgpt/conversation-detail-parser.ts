...
import { normalizeObsidianMathDelimiters } from "./session-config";
...
function extractMessageContent(message: UnknownRecord): string {
  ...
  collectTextFragments(message.content, fragments, new WeakSet<object>());
...
function renderMessageBody(message: UnknownRecord, refs: Map<string, ConversationFileReference>): string {
  ...
  switch (readString(content.content_type)) {
    case "text":
...
function renderMessageMarkdown(
  ...
  const metadata = toRecord(message.metadata);
  body = applyChatGptContentReferencesAsFootnotes(body, metadata?.content_references, footnoteRegistry, {
    finalizeText: false,
  }).text;
