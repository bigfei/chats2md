...
test("disabled account metadata remains persisted in stored settings", () => {
  const settings = createSettings();
  assert.equal(settings.accounts[0]?.disabled, true);
  assert.equal(settings.accounts[1]?.disabled, false);
});
