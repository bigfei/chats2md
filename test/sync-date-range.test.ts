...
test("filterConversationSummariesByUpdatedDateRange keeps inclusive boundaries", () => {
  ...
  const filtered = filterConversationSummariesByUpdatedDateRange(summaries, "2026-03-01", "2026-03-31");
...
test("invalid updated_at values are ignored by span and range filtering", () => {
  ...
  const filtered = filterConversationSummariesByUpdatedDateRange(summaries, "2026-01-01", "2026-01-31");
  assert.deepEqual(
    filtered.map((summary) => summary.id),
    ["early"],
  );
...
test("filterConversationSummariesByLatestCount rejects invalid count", () => {
  ...
  assert.throws(() => filterConversationSummariesByLatestCount(summaries, 0), /positive integer/);
});
